...
export function useTokenList(chainId: ChainId, tokenListUri?: string, formattingOptions: Partial<CurrencyFormatOptions> = {}): Token[] | undefined {
    ...
    useEffect(() => {
        const getTokenList = async () => {
            try {
                ...
                const tokens = _tokenList.tokens.forEach((token: { name: string; symbol: string; address: string; decimals: number | undefined }) => {
