...
With the custom chain config created and saved in `tutorial-chain.ts` file, you can include it in `useDApp` Config object that was introduced in [Getting Started example](../getting-started#example).
