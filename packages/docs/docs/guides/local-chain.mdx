...
In this tutorial, we will go through the steps of connecting to a local Chain.
...
```tsx
  ...
  const config = {
    ...
    readOnlyUrls: {
      [Localhost.chainID]: 'http://127.0.0.1:8545',//paste ganache address
    },
  }
```
...
Deploy multicall contract to the ganache and paste its address to file local.ts
...
export const Localhost: Chain = {
  ...
  isTestChain: true,
  isLocalChain: true,
  multicallAddress: '0xmulticallAddress',//paste local multicall address
...
To read from blockchain just add the queryOptions object to specified call.
...
## Summary
In this tutorial, we went through the steps required for using useDApp with a local chain.

We modify the local chain by adding local multicall address.
