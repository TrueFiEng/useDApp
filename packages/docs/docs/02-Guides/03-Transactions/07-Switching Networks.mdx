...
It means that there the network you are trying to switch to, is known, but there is no stable public RPC endpoint in useDApp. Known networks are listed [here](https://github.com/TrueFiEng/useDApp/tree/master/packages/core/src/model/chain).
...
Because of that, `switchNetwork` will throw this error because a network cannot be added automatically to a browser Wallet without an RPC endpoint.

**Solution**

1. If you have an RPC endpoint, you can supply it to useDApp config.
