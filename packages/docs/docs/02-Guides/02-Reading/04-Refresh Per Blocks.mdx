...
To disable refreshing calls per block, just pass `refresh` parameter as `never` to config, it will fetch data only once, example configuration below:
...
To enable refreshing calls per specified number of blocks, just pass `refresh` parameter as specified number to config, it will refresh each 10 mined blocks, example configuration below:
