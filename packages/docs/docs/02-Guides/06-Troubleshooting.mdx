...
The chain you are using has not Multicall v2 address passed into the `Config`.
Using version 1 of Multicall is not recommended, as it is not as efficient as version 2
and failing one of the calls will cause the whole batch to fail.

You can provide the Multicall v2 address of your chain into the `Config` to use it.
